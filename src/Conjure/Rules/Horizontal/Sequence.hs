...
rule_Comprehension_GetSubstring :: Rule
rule_Comprehension_GetSubstring = "sequence-getSubstring" `namedRule` theRule where
    theRule (Comprehension body gensOrConds) = do
        (gofBefore, (pat, expr), gofAfter) <- matchFirst gensOrConds $ \ gof -> case gof of
            Generator (GenInExpr pat@Single{} expr) -> return (pat, expr)
            _ -> na "rule_Comprehension_getSubstring"
        (seq, start, len) <- match opGetSubstring expr
        TypeSequence{} <- typeOf seq
        let upd val old = lambdaToFunction pat old val
        return
            ( "Mapping over the substring of a sequence"
            , do
                (jPat, j) <- quantifiedVar
                let val = [essence| (&j[1]-&start+1,&j[2]) |]
                return $ Comprehension
                        (upd val body)
                        $  gofBefore
                        ++ [ Generator (GenInExpr jPat seq)
                           , Condition [essence| &j[1] <= &start + &len |]
                           , Condition [essence| &j[1] >= &start |]
                           ]
                        ++ transformBi (upd val) gofAfter
            )
    theRule _ = na "rule_Comprehension_PreImage"
